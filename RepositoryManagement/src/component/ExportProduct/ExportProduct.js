...
class ExportProduct extends Component {
  ...
  componentDidMount(){
    ...
    this.apiService.getListShippers(this.props.user.RepositoryID, this.props.access_token).then((data)=>{
      this.setState({
          shippers: data,
          shipperID: data[0].ID,
          shipperFullName: data[0].FullName
      })
    }).catch((e) => {
      let error = 'Có lỗi xảy ra. Vui lòng thử lại'
      if (e.status === 401) {
  ...
  handleNewProduct = () =>{
    ...
    if(this.state.quantity >= this.state.newProduct.Quantity){
      this.setState({
        error: "Số lượng phải nhỏ hơn số lượng sản phẩm có trong kho"
      })
      return
    }
  ...
  hanleExport = () =>{
    ...
    if(this.state.shipperID !== null)
    {
      let exporterID = this.props.user.ID
      if(this.state.listNewProduct.length > 0){
        this.apiService.exportProduct(
          {
            exporterID: exporterID,
            repositoryID: this.props.user.RepositoryID,
            ProductExports: this.state.listNewProduct,
          ...
          this.setState({
            notification1: false,
            notification: "Xuất kho thành công",
            isLoading: false,
            listNewProduct: [],
            listProduct: [],
            reason: '',
      ...
      alert("Yêu cầu chọn nhân viên giao hàng", "Phản hồi")
    }
  }
    ...
    return(
      <div className="product-status mg-b-15">
        <div className="container-fluid">
              ...
              <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div className="product-status-wrap drp-lst">
                  ...
                  <div className="option-choose">
                    <div className="form-group chooseShipper">
                      <label htmlFor="exampleInputShipper">Nhân Viên Giao Hàng</label>
                      <select className="form-control option" id="exampleInputShipper" onChange={this.handleShipperChange}>
                      {this.state.shippers && this.state.shippers.map((temp, i)=>(
                          <option key={i} value={temp.ID} >{temp.FullName}</option>
                      ))}
                      </select>
                    </div>
                    ...
                    <table>
                      <thead>
                        ...
                        <tr key={i}>
                          ...
                          <td>
                            <button type="button" class="pd-setting-ed" data-toggle="modal" data-target={"#"+temp.ID}><i className="fa fa-pencil-square-o" aria-hidden="true"></i></button>
            ...
            <div className="col-lg-12 export">
              <div className="payment-adress">
                <div className="row">
                  <div className="col-md-7 ">
                    <label htmlFor="exampleInput" className="reason-label">Lý Do Xuất Kho</label>
                    <input type="text" className="form-control inputExport" id="exampleInput" placeholder="Vd: Xuất hàng cho nhân viên giao hàng" onChange={this.handleReasonChange}/>
                  </div>
                  <div className="col-md-5">
                      <div className="col-lg-12">
                        <div className="col-md-2">
                          <button type="submit" className="btn btn-primary waves-effect waves-light" onClick={this.hanleExport}>
                          {this.state.isLoading ? <><span style={{width: '80%', height: '90%'}} className="loader"></span>Lưu</> : 'Lưu'}
                          </button>
                        </div>
                        <div className="col-md-6" style={{display: stylebtn}}>
